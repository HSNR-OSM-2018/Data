...
public class FilteredDataProvider extends SimpleDataProvider {
    ...
    /**
     * Get the start and destination nodes for the given geo-coordinates
     *
     * @param startLatitude        Latitude of start position
     * @param startLongitude       Longitude of start position
     * @param destinationLatitude  Latitude of destination position
     * @param destinationLongitude Longitude of destination position
     * @return Graph together with the start- and destination-node
     */
    public PathGraphContainer getPathGraphContainer(double startLatitude, double startLongitude, double destinationLatitude, double destinationLongitude) {
        logger.info("Starting node-search for: startLatitude = [" + startLatitude + "], startLongitude = [" + startLongitude + "], destinationLatitude = [" + destinationLatitude + "], destinationLongitude = [" + destinationLongitude + "]");
        ...
        logger.info("Finished node-search in " + (time / 1000) + " seconds");
        return new PathGraphContainer(startNode, destinationNode, mGraph);
    }
}